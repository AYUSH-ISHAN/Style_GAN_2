...
from layers import FusedLeakyReLU, fused_leaky_relu, upfirdn2d, conv2d_gradfix
...
##  Some golabl parameters declaration..

p_augment = 0  ## (came after removing the args.augment_p part)
...
class DRM(Dataset):
    def __init__(self, path, transform, resolution=256):
        self.env = lmdb.open(
            "/content/drive/MyDrive/Stylegan2-pytorch/lmdb",
            max_readers=32,
            readonly=True,
            lock=False,
            readahead=False,
...
###  Atricficial Augmenetation part (Remove them on basis of computational requirements 
###  and accuracy achieved bu model)
...
def SG2_training(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    if args.augment and p_augment == 0:
        ada_augment = AdaptiveAugment(target_aug, args.ada_length, 8, device)
...
if __name__ == "__main__":
    ...
    # parser.add_argument(
    #     "--size", type=int, default=32, help="image sizes for the model"   ## change the default size to 32 
    # )
    parser.add_argument(
    ...
    args.distributed = False  # dealing with single GPU.
    ...
    # if args.distributed:    (since we are using only 1 GPU)
    ...
    dataset = DRM(args.path, transform, size_of_image)
    ...
    SG2_training(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device)
