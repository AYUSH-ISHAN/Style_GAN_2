...
def SG2_training(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    sample_z = torch.randn(NUMBER_OF_SAMPLE, args.latent, device=device)
    ...
    for idx in pbar:
        ...
        noise = mixing_noise(args.batch, args.latent, MIXING, device)
        ...
        if g_regularize:
            path_batch_size = max(1, args.batch // PBS)
            noise = mixing_noise(path_batch_size, args.latent, MIXING, device)
